...
import { tipTapJsonToMarkdown } from "@/lib/query/json-to-markdown";
...
.handler(async ({input, context, errors}) => {
    //fetch the base message: from it we create unput for the llm
    ...
    if(replies.length>0){
        ...
        //array of intial messag,parent message. and replies
...
"Return ONLY the rewritten content. No preamble, headings, or closing remarks."
    ...
    const result = streamText({
        ...
        messages:[
            {
                role:'user',
                content:'Please rewrite and improve the following conent:'
            },
            {
                role:'user',
                content:markdown,
